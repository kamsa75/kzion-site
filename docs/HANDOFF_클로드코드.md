...
- ✅ **1단계 DB** — `bulletin_schema.sql`(테이블 7개) + 시드 실행 완료. 검증 쿼리 통과
  (7/19 주보 행사계획 5줄, 기도/봉헌 로테이션, 권/호 41권29호 전부 대조 일치)
- ✅ **2단계 Edge Function** — **별도 함수 `bt`로 분리 배포**(B11 — api 무변경).
  대시보드 via Editor 배포, Verify JWT off, 응답 확인 완료. 액션 12개
- ⬜ **3단계 화면** `/bt` — 다음 작업. 최종엔 PPT 페이지에 [주보] 탭으로 통합(사용자 요청)
